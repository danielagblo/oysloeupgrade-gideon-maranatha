...
async function seedDatabase() {
  ...
  try {
    ...
    console.log(
      `   👨‍💼 Test Admin: ${testAdmin.username} (ID: ${testAdmin.id})`
    );
...
async function clearExistingData() {
  console.log(
    "   Note: Skipping data clearing to avoid foreign key constraints."
  );
...
async function createTestAdmin(): Promise<AdminUser> {
...
async function createCategories(): Promise<Category[]> {
  ...
  if (existingCategories.length > 0) {
    console.log(
      `   Categories already exist (${existingCategories.length} found), skipping creation`
    );
...
async function createProducts(
  ...
  if (existingProducts.length > 0) {
    console.log(
      `   Products already exist for test user (${existingProducts.length} found), skipping creation`
    );
  ...
  const productsData = [
    {
      ...
      description:
        "Latest iPhone with advanced features. Barely used, comes with original box and accessories.",
      price: 1199.99,
    ...
    },
    {
      name: "MacBook Pro M3",
      description:
        "Powerful laptop for professional work. 16GB RAM, 512GB SSD. Perfect condition.",
      price: 2499.99,
      categoryId: categories[0].id, // Electronics
    ...
    },
    {
      ...
      description:
        "Well maintained Honda Civic with low mileage. Single owner, full service history.",
      price: 18500.0,
    ...
    },
    {
      ...
      description:
        "Beautiful 2 bedroom apartment in downtown. Fully furnished, parking included.",
      price: 2500.0,
    ...
    },
    {
      ...
      description:
        "Authentic designer handbag in excellent condition. Comes with dust bag and authenticity card.",
      price: 899.99,
    ...
    },
    {
      ...
      description:
        "High-end gaming PC with RTX 4070, i7 processor, 32GB RAM. Perfect for gaming and content creation.",
      price: 3299.99,
      categoryId: categories[0].id, // Electronics
      subcategoryId: null, // Will be set to Accessories
  ...
  const subcategoryRepo = AppDataSource.getRepository(Subcategory);
  const electronicsSubs = await subcategoryRepo.find({
    where: { categoryId: categories[0].id },
  });
  const vehiclesSubs = await subcategoryRepo.find({
    where: { categoryId: categories[1].id },
  });
  const realEstateSubs = await subcategoryRepo.find({
    where: { categoryId: categories[2].id },
  });
  const fashionSubs = await subcategoryRepo.find({
    where: { categoryId: categories[3].id },
  });
  ...
  for (let i = 0; i < productsData.length; i++) {
    ...
    if (i === 0) subcategoryId = subcategoryMap.smartphones; // iPhone
      ...
      subcategoryId = subcategoryMap.clothing; // Handbag (fashion)
...
async function createSupportCases(user: User, admin: AdminUser) {
  ...
  const existingCases = await supportCaseRepo.find({
    where: { userId: user.id },
  });
  if (existingCases.length > 0) {
    console.log(
      `   Support cases already exist for test user (${existingCases.length} found), skipping creation`
    );
  ...
  const casesData = [
    {
      ...
      messages: [
        {
          content:
            "Hi, I'm having trouble listing my iPhone for sale. The images aren't uploading properly.",
          isFromAdmin: false,
        },
        {
          content:
            "Hello! I'll help you with that. Can you try clearing your browser cache and trying again?",
          isFromAdmin: true,
        },
        {
          content:
            "Thanks! That worked. But now I can't set the price. It keeps showing an error.",
          isFromAdmin: false,
        },
      ],
    },
    {
      ...
      messages: [
        {
          content:
            "I sold my MacBook yesterday but haven't received the payment yet. It's been 24 hours.",
          isFromAdmin: false,
        },
        {
          content:
            "I'm investigating this payment issue. Can you provide the transaction ID?",
          isFromAdmin: true,
        },
      ],
    },
    {
      ...
      messages: [
        ...
        {
          content:
            "Account verification typically takes 24-48 hours. You'll receive an email once it's complete.",
          isFromAdmin: true,
        },
        {
...
async function createReviews(products: Product[], user: User) {
  ...
  if (existingReviews.length > 0) {
    console.log(
      `   Reviews already exist for test user (${existingReviews.length} found), skipping creation`
    );
    return;
  }
  ...
  const reviewsData = [
    {
      ...
      rating: 5,
      comment:
        "Excellent product! Exactly as described. Fast shipping and great communication.",
    },
    {
      productId: products[1].id, // MacBook
      rating: 4,
      comment:
        "Great laptop, very satisfied with the purchase. Minor wear but nothing major.",
    },
    {
      productId: products[2].id, // Honda Civic
      rating: 5,
      comment:
        "Amazing car! Well maintained and exactly as advertised. Highly recommend!",
    },
  ];
...
async function createFavorites(products: Product[], user: User) {
  ...
  const existingFavorites = await favoriteRepo.find({
    where: { userId: user.id },
  });
  if (existingFavorites.length > 0) {
    console.log(
      `   Favorites already exist for test user (${existingFavorites.length} found), skipping creation`
    );
    return;
  }
