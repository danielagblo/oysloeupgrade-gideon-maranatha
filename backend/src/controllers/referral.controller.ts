...
export class ReferralController {
    ...
    try {
      ...
      if (!referrerId || !referredUserId || !referralCode) {
        throw new BadRequestError(
          "referrerId, referredUserId, and referralCode are required"
        );
      }
  ...
  async getUserStats(req: Request, res: Response) {
    try {
      ...
      const stats = await this.referralService.getUserReferralStats(
        req.user.id
      );
  ...
  async getUserReferralHistory(req: Request, res: Response) {
    try {
      ...
      const { referrals, total } =
        await this.referralService.getUserReferralHistory(
          req.user.id,
          Number(page),
          Number(limit)
        );
  ...
  async getUserRedemptionHistory(req: Request, res: Response) {
    try {
      ...
      const { redemptions, total } =
        await this.referralService.getUserRedemptionHistory(
          req.user.id,
          Number(page),
          Number(limit)
        );
