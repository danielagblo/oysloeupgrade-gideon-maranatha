...
export class AnalyticsController {
  ...
  async trackProductView(req: Request, res: Response) {
    try {
      const { id: productId } = (req.validated?.params || req.params) as Partial<
        Record<string, string>
      >;
      const userId = req.user?.id;
      const metadata = req.validated?.body || req.body;

      await this.analyticsService.trackProductView(userId, productId as string, metadata);
      ...
      res.status(statusCode).json({
        success: false,
        message: error instanceof BadRequestError ? error.message : 'Failed to track product view',
      });
    }
  }
  ...
  async trackProductClick(req: Request, res: Response) {
    try {
      ...
      await this.analyticsService.trackProductClick(userId, productId as string, metadata);
      ...
      res.status(statusCode).json({
        success: false,
        message: error instanceof BadRequestError ? error.message : 'Failed to track product click',
      });
    }
  }
  ...
  async getUserEngagement(req: Request, res: Response) {
    try {
      ...
      const { days } = (req.validated?.query || req.query) as Partial<Record<string, string>>;
      ...
      const stats = await this.analyticsService.getUserEngagementStats(userId, numDays);
      ...
      res.status(statusCode).json({
        success: false,
        message: error instanceof BadRequestError ? error.message : 'Failed to get user engagement',
      });
    }
  }
  ...
  async getProductAnalytics(req: Request, res: Response) {
    try {
      const { productId, days } = (req.validated?.query || req.query) as Partial<
        Record<string, string>
      >;
      ...
      res.status(statusCode).json({
        success: false,
        message:
          error instanceof BadRequestError ? error.message : 'Failed to get product analytics',
      });
    }
  }
  ...
  async getTopProducts(req: Request, res: Response) {
    try {
      ...
      const topProducts = await this.analyticsService.getTopProducts(numLimit, numDays);
      ...
      res.status(statusCode).json({
        success: false,
        message: error instanceof BadRequestError ? error.message : 'Failed to get top products',
      });
    }
  }
  ...
  async getTrendingSearches(req: Request, res: Response) {
    try {
      ...
      const trendingSearches = await this.analyticsService.getTrendingSearchTerms(
        numLimit,
        numDays
      );
      ...
      res.status(statusCode).json({
        success: false,
        message:
          error instanceof BadRequestError ? error.message : 'Failed to get trending searches',
      });
    }
  }
}
