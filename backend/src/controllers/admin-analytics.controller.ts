...
export const getDashboardOverview = async (_req: Request, res: Response, _next: NextFunction) => {
  try {
    ...
    // Revenue (placeholder - would need actual transaction data)
    ...
    const resolvedToday = await supportCaseRepository
      ...
      .getCount();
    const avgResponseTime = 0; // TODO: calculate from case assignment and resolution times
...
export const getUserAnalytics = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // Build time series data for registrations
    ...
    // Device types (placeholder - would need device tracking data)
...
export const getAdsAnalytics = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // Build time series data for postings
    ...
    // Build time series data for approvals
    ...
    // Build time series data for suspensions
...
export const getRevenueAnalytics = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // Placeholder - would need actual transaction/revenue data
    res.json({
      success: true,
      data: {
        total: 0,
...
export const getSupportAnalytics = async (_req: Request, res: Response, next: NextFunction) => {
  try {
    const supportCaseRepository = AppDataSource.getRepository(SupportCase);
    const supportCaseAssignmentRepository = AppDataSource.getRepository(SupportCaseAssignment);
    ...
    resolvedCasesWithDates.forEach((c) => {
      if (c.resolvedAt && c.createdAt) {
        const resolutionTime =
          (new Date(c.resolvedAt).getTime() - new Date(c.createdAt).getTime()) / (1000 * 60 * 60); // hours
        totalResolutionTime += resolutionTime;
        validResolutions++;
      }
    });

    const avgResolutionTime = validResolutions > 0 ? totalResolutionTime / validResolutions : 0;
    ...
    const agentPerformanceRaw = await supportCaseAssignmentRepository
      ...
      .addSelect('COUNT(CASE WHEN assignment.unassignedAt IS NOT NULL THEN 1 END)', 'casesResolved')
      .groupBy('admin.id')
