...
export async function sign(req: Request, res: Response) {
  ...
  if (adminUploadTypes.includes(parsed.kind)) {
    ...
    if (parsed.kind === 'avatar') {
      if (req.user.id !== parsed.entityId && !req.user.isStaff && !req.user.isSuperuser) {
...
export async function confirm(req: Request, res: Response) {
  ...
  if (parsed.kind === 'avatar') {
    ...
    const documentType =
      (parsed.context?.documentType as ApplicationDocument['documentType']) || 'other';
...
export async function destroy(req: Request, res: Response) {
  ...
  if (!req.user?.id) return res.status(401).json({ error: 'Unauthorized' });
  if (!publicId || !kind || !entityId) return res.status(400).json({ error: 'Missing parameters' });

  if (kind === 'avatar') {
    if (req.user.id !== entityId && !req.user.isStaff && !req.user.isSuperuser) {
      return res.status(403).json({ error: 'Forbidden' });
    }
    const userRepo = AppDataSource.getRepository(User);
    const user = await userRepo.findOne({ where: { id: entityId } });
    if (!user) return res.status(404).json({ error: 'User not found' });
    if (user.avatarPublicId !== publicId) return res.status(404).json({ error: 'Media not found' });
