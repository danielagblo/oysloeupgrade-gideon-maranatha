...
export class User {
  ...
  @OneToMany(
    () => Referral,
    (r: Referral) => r.referrer
  )
  referralsGiven?: Referral[];

  @OneToMany(
    () => Referral,
    (r: Referral) => r.referredUser
  )
  referralsReceived?: Referral[];

  @OneToMany(
    () => ReferralRedemption,
    (rr: ReferralRedemption) => rr.user
  )
  referralRedemptions?: ReferralRedemption[];

  @OneToMany(
    () => FCMDevice,
    (fd: FCMDevice) => fd.user
  )
  fcmDevices?: FCMDevice[];

  @OneToMany(
    () => CouponRedemption,
    (cr: CouponRedemption) => cr.user
  )
  ...
  @OneToMany(
    () => NotificationHistory,
    (nh: NotificationHistory) => nh.user
  )
  notificationHistory?: NotificationHistory[];

  @OneToMany(
    () => UserAnalytics,
    (ua: UserAnalytics) => ua.user
  )
  analytics?: UserAnalytics[];

  @OneToMany(
    () => SearchHistory,
    (sh: SearchHistory) => sh.user
  )
  searchHistory?: SearchHistory[];

  @OneToMany(
    () => RecentlyViewed,
    (rv: RecentlyViewed) => rv.user
  )
