...
const getRecentlyViewedSchema = z.object({
  query: z.object({
    limit: z.coerce.number().int().positive().optional().describe('Number of products to return'),
  }),
});
...
const enhancedSearchSchema = z.object({
  query: z.object({
    ...
    limit: z.coerce.number().int().positive().optional().describe('Number of results per page'),
  }),
});
...
router.delete('/history', authenticate, searchController.clearSearchHistory.bind(searchController));
