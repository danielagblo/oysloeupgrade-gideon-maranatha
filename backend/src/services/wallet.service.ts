...
export type TransactionType = 'credit' | 'debit' | 'referral' | 'coupon' | 'purchase' | 'refund';
...
export class WalletService {
  ...
  async creditWallet(
    ...
    return await AppDataSource.transaction(async (manager) => {
      ...
      logInfo(`Credited ${amount} to user ${userId} wallet. New balance: ${newBalance}`);
  ...
  async debitWallet(
    ...
    return await AppDataSource.transaction(async (manager) => {
      ...
      logInfo(`Debited ${amount} from user ${userId} wallet. New balance: ${newBalance}`);
  ...
  async transferFunds(
    ...
    return await AppDataSource.transaction(async (manager) => {
      ...
      await manager.update(Wallet, { userId: fromUserId }, { balance: fromNewBalance });
      await manager.update(Wallet, { userId: toUserId }, { balance: toNewBalance });
      ...
      logInfo(`Transferred ${amount} from user ${fromUserId} to user ${toUserId}`);
  ...
  async processReferralBonus(
    ...
    return await this.creditWallet(referrerId, bonusAmount, 'referral_bonus', referredUserId, {
      referredUserId,
      bonusType: 'referral',
      amount: bonusAmount,
    });
  ...
  async processCouponDiscount(
    ...
    return await this.creditWallet(userId, discountAmount, 'coupon_discount', couponId, {
      couponId,
      orderId,
      discountType: 'coupon',
  ...
  async processRefund(
    ...
    return await this.creditWallet(userId, amount, reason, originalTransactionId, {
  ...
  async getWalletSummary(userId: string): Promise<{
    ...
    return {
      ...
      transactionCount: (Number(creditStats.count) || 0) + (Number(debitStats.count) || 0),
    };
  }
}
