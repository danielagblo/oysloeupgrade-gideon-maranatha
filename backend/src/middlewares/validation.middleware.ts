...
const isZodSchema = (v: unknown): v is ZodTypeAny =>
  !!v &&
  typeof v === "object" &&
  "parseAsync" in (v as Record<string, unknown>);
...
const isDirectTrio = (v: unknown): v is Trio =>
  !!v &&
  typeof v === "object" &&
  (hasOwn(v as object, "body") ||
    hasOwn(v as object, "query") ||
    hasOwn(v as object, "params"));
...
export function validateRequest(bundle: Bundle) {
  ...
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (shape.query)
        req.query = (await shape.query.parseAsync(req.query || {})) as ParsedQs;
      if (shape.params)
        req.params = (await shape.params.parseAsync(
          req.params
        )) as ParamsDictionary;
      next();
    } catch (err) {
